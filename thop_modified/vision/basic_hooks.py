...
def count_convNd(m: _ConvNd, x: (torch.Tensor,), y: torch.Tensor):
    ...
    batch_size, output_channels, output_height, output_width = y.size()

    kernel_ops = torch.zeros(m.weight.size()[2:]).numel()  # Kw x Kh
    bias_ops = 1 if m.bias is not None else 0

    num_weight_params = (m.weight.data != 0).float().sum()
    # N x Cout x H x W x  (Cin x Kw x Kh + bias)
    total_ops = output_height * output_width * batch_size * (num_weight_params + bias_ops *output_channels)
...
def count_linear(m, x, y):
    ...
    num_elements = y.numel()
    total_ops = total_mul * num_elements * num_weight_params

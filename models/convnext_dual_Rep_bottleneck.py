...
import sys, os
sys.path.append('/home/sliu/Projects/cutlass/examples/19_large_depthwise_conv2d_torch_extension')
...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, kernel_size=(7,7), LoRA=None, bn=True, attention=False):
        ...
        self.large_kernel = Bottleneck_attention(in_channels=dim, out_channels=dim,
                                                   kernel_size=kernel_size[0],
                                                   stride=1, groups=dim, small_kernel=kernel_size[1],
                                                   small_kernel_merged=False, LoRA=LoRA, bn=bn)

        # self.dwconv1 = DepthWiseConv2dImplicitGEMM(dim, (kernel_size[0], kernel_size[1]), bias=True) # depthwise conv
        # self.dwconv2 = DepthWiseConv2dImplicitGEMM(dim, (kernel_size[1], kernel_size[0]), bias=True) # depthwise conv
        # self.dwconv_sparse = DepthWiseConv2dImplicitGEMM(dim, (kernel_size[0], kernel_size[0]), bias=True) # depthwise conv
...
class SLAK_Attention(nn.Module):
    r""" SLAK_Attention
    Args:
...
@register_model
def SLAK_Attention_tiny(pretrained=False, **kwargs):
    model = SLAK_Attention(depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], **kwargs)
    if pretrained:
...
@register_model
def SLAK_Attention_small(pretrained=False, **kwargs):
    model = SLAK_Attention(depths=[3, 3, 27, 3], dims=[96, 192, 384, 768], **kwargs)
    if pretrained:
...
@register_model
def SLAK_Attention_base(pretrained=False, in_22k=False, **kwargs):
    model = SLAK_Attention(depths=[3, 3, 27, 3], dims=[128, 256, 512, 1024], **kwargs)
    if pretrained:
...
@register_model
def SLAK_Attention_large(pretrained=False, in_22k=False, **kwargs):
    model = SLAK_Attention(depths=[3, 3, 27, 3], dims=[192, 384, 768, 1536], **kwargs)
    if pretrained:

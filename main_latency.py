...
import models.convnext_latency
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='convnext_tiny_Rep', type=str, metavar='MODEL',
    ...
    parser.add_argument('--kernel-size', nargs="*", type=int,
                        default = [31,31], help='kernel size of attention (default: [31,29,27,13,3])')
    ...
    parser.add_argument('--resolution', type=int, default=16, help='set the width factor of the model')
...
def main(args):
    ...
    mask=None
    if args.sparse:
        mask = Masking(optimizer, train_loader=data_loader_train, prune_mode=args.prune, prune_rate_decay=None, growth_mode=args.growth, redistribution_mode=args.redistribution, args=args)
        mask.add_module(model)

    # device = torch.device("cuda")
    dummy_input = torch.randn(64, 384, args.resolution, args.resolution, dtype=torch.float).to(device)
